...
class FollowSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        user = generics.get_object_or_404(
            User, username=data.get("following").username
        )
        follow = Follow.objects.filter(
            user=self.context.get("request").user, following=user
        ).exists()
        ...
        if follow:
            raise serializers.ValidationError(f"Вы уже подписаны на {user}!")
        return data
